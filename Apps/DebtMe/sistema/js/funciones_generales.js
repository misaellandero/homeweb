               ...
               $.each(respuesta, function(key, value) { 
                      ...
                      if (value.nombre == null) {
                        ...
                        $('#divTablaFactura').html(tabla); 
                            ...
                            cerrar = '<button type="button" class="btn btn-cerrar-busqueda inversePrimarySet btn-outline"> <i class="fa fa-times"></i> cerrar</button>';
...
//cargarDatosUsuario
   ...
   function cargarMovimientosEfectivo(){
    ...
    function cargarDireccionesUsuario(id_usuario,tienda) { 
  
        if (tienda == "Si") {
         var url = 'sistema/php/users/consulta_direccion_usuario.php';
        } else{
        var url = 'php/users/consulta_direccion_usuario.php';
        }

  

        var tabla = ' <table id="tablaDirecciones" class="table datatable resposive" width="100%" > '
             + '<thead> '
             + '<tr role="row">'
             + '<th><i class="fa fa-user"></i> Nombre</th>'
			 + '<th><i class="fa fa-hashtag"></i> Numero</th>'
			 + '<th><i class="fa fa-road"></i> Calle</th> '
			 + '<th><i class="fas fa-city"></i> Ciudad</th>'
			 + '<th><i class="fa fa-map"></i> Estado</th>'
        ...
        $('#div_tabla_direcciones').html(tabla); 
        ...
        function(data) {   
           console.log(data);
            $.each(data, function(index, record) {
                if ($.isNumeric(index)) {
                    var eliminar = '<button type="button" class="btn-eliminar-dir btn btn-danger" data-id=' + record.id + ' ><i class=" fa fa-trash"></i> Borrar</button>';
            
                    //id de la direccion
                    var row = $("<tr data-id='" + record.id + "' role='row' class='odd'/>"); 

                    $("<td />").html(record.nombre ).appendTo(row);
                    $("<td />").html(record.nexterior ).appendTo(row); 
                    $("<td />").html(record.calle ).appendTo(row); 
                    $("<td />").html(record.ciudad ).appendTo(row); 
                    $("<td />").html(record.estado ).appendTo(row);
                    $("<td />").html(record.cp ).appendTo(row); 
                    $("<td />").html(eliminar ).appendTo(row); 
                    row.appendTo("#tablaDirecciones"); 
                 }
            }); 
...
function eliminarDireccion(id,url) {
  ...
  $.ajax({
      ...
      success :  function(response){
            ...
            alert(response); 
            cargarDireccionesUsuario("mismo_login","Si");
