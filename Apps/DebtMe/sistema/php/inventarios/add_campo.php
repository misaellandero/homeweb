<?php
 $conn = require_once '../conexion.php';
  ...
  $precioCampo = trim($_POST['precioCampo']);
   ...
   $result->execute([$nombreCampo]);
    ...
    if ($result->rowCount() > 0) { 
        echo "$nombreCampo ya existe" ; // wrong details
          ...
          $sql = "INSERT INTO `campos`(`nombre`, `tipo`, `id_unidad`, `precio`,`fijo`) VALUES (?,?,?,?,0) ";
          $result = $conn->prepare($sql); 
          	if ($result->execute([$nombreCampo,$tipoCampo,$unidadCampo,$precioCampo])) {
